...
const DepartmentRejectedVoucher = () => {
  ...
  const { data: rejectedVouchers = [], isPending: isRejectedLoading } = useQuery({
    queryKey: ["departmentRejectedVouchers", department?._id],
    queryFn: async () => {
      try {
    ...
    <div>
      <PageFrame>
        <YearWiseTable
          ...
          search={true}
          tableTitle={`${departmentTitle} Rejected Voucher`}
          data={tableData}
          columns={columns}
...
export default DepartmentRejectedVoucher;