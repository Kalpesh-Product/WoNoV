...
const Unauthorized = ({
  ...
  const redirectTarget = redirectTo || location?.state?.redirectTo;
  const pageTitle = location?.state?.title || title;
  const pageMessage = location?.state?.message || message;
  ...
  }, [navigate, redirectDelayMs, redirectTarget]);
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-gray-100 px-4">
      <div className="bg-white shadow-lg rounded-2xl p-8 max-w-md text-center">
        ...
        <p className="text-gray-600 mt-2">{pageMessage}</p>
        <p className="text-gray-500 mt-4 text-sm">
          Redirecting you in{" "}
          <span className="font-medium">{Math.ceil(redirectDelayMs / 1000)} seconds</span>...
        </p>
        <div className="mt-6 flex flex-col gap-3">
          ...
          >
            {redirectTarget ? "Go to Dashboard" : "Go Back Now"}
          </button>
          {/* <button
