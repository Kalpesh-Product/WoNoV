...
const PerformanceReportKraKpa = () => {
  ...
  const handleTaskSearchChange = (value) => {
    setTaskSearch(value);
  ...
  const {
    departmentName: routeDepartmentName,
    reportType: routeReportType,
    reportStatus: routeReportStatus,
  } = useParams();
  ...
  const getDepartmentPathSegment = (departmentName) =>
    encodeURIComponent(departmentName || "");
  const decodedRouteDepartmentName = decodeURIComponent(
    routeDepartmentName || "",
  );

  const activeTypeTab = routeReportType === "KPA" ? "KPA" : "KRA";
  const activeStatusTab =
    routeReportStatus === "Pending" ? "Pending" : "Completed";
  ...
  const activeDepartmentId =
    selectedDepartmentByRoute?.id || departments[0]?.id || "";
  ...
  useEffect(() => {
    ...
    if (
      ...
      navigate(
        `/app/performance/report-KRA-KPA/${getDepartmentPathSegment(nextDepartment.name)}/${nextType}/${nextStatus}`,
        { replace: true },
      );
    }
  }, [
  ...
  const navigateToReport = (departmentName, type, status) => {
    ...
    navigate(
      `/app/performance/report-KRA-KPA/${getDepartmentPathSegment(departmentName)}/${type}/${status}`,
    );
  };
  ...
  const { data: reportData = [], isPending } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (response.data?.data || []).sort(
        (a, b) =>
          new Date(b.completionDate || b.assignedDate) -
          new Date(a.completionDate || a.assignedDate),
      );
    },
  });
  ...
  const reportColumns = [
    ...
    {
      headerName:
        activeStatusTab === "Completed" ? "Completed By" : "Assigned To",
  ...
  return (
    <div className="flex flex-col gap-4">
      <Tabs
        ...
        onChange={(_, newValue) =>
          navigateToReport(
            departments.find((department) => department.id === newValue)?.name,
            activeTypeTab,
            activeStatusTab,
          )
        }
      ...
      <Tabs
        value={activeTypeTab}
        onChange={(_, newValue) =>
          navigateToReport(selectedDepartment?.name, newValue, activeStatusTab)
        }
      ...
      >
        {completedTaskView && (
          <div className="grid grid-cols-1 gap-4">
            <DetalisFormatted
              title={activeTypeTab}
              detail={completedTaskView?.taskName}
            />
            <DetalisFormatted
              title={"Department"}
              detail={completedTaskView?.department}
            />
            <DetalisFormatted
              title={"Completed By"}
              detail={completedTaskView?.completedBy || "-"}
            />
            <DetalisFormatted
              title={"Assigned Date"}
              detail={
                completedTaskView?.assignedDate
                  ? humanDate(completedTaskView.assignedDate)
                  : "-"
              }
            />
            <DetalisFormatted
              title={"Due Date"}
              detail={
                completedTaskView?.dueDate
                  ? humanDate(completedTaskView.dueDate)
                  : "-"
              }
            />
            <DetalisFormatted
            ...
            <DetalisFormatted
              title={"Status"}
              detail={completedTaskView?.status || "-"}
            />
            <DetalisFormatted
              title={"Comment"}
              detail={completedTaskView?.comment || "-"}
            />
          </div>
        )}
      </MuiModal>
