...
const EnergyMonthlyReadingHistory = () => {
  ...
  useEffect(() => {
    let active = true;
    axiosPrivate
      .get(`/api/maintenance/energy-monthly-history/${module}/${readingId}`)
  ...
  const columns = [
    ...
    { field: "totalConsumption", headerName: "Total Consumption (KWH)", minWidth: 190, flex: 1 },
    ...
    <div className="p-4">
      <PageFrame>
        <AgTable
          ...
          exportData
          hideFilter
          tableTitle={`${module === "dtc" ? "DTC" : "ST"} ENERGY MONTHLY READING HISTORY`}
          headerActions={<Button variant="outlined" onClick={() => navigate(-1)}>Back</Button>}
        />
      </PageFrame>
    </div>
  );
