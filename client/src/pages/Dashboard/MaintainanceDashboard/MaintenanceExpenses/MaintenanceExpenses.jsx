...
const MaintenanceExpenses = () => {
  ...
  const stackedRoundedMax = useMemo(() => {
    const fiscalYears = [
      ...new Set(stackedExpenseRawSeries.map((series) => series.group)),
    ];

    const maxValue = fiscalYears.reduce((max, fiscalYear) => {
      const actualSeries = stackedExpenseRawSeries.find(
        (series) =>
          series.group === fiscalYear && series.name === "Actual Amount"
      );
      const projectedSeries = stackedExpenseRawSeries.find(
        (series) =>
